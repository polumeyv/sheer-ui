import type { Getter, MaybeBoxOrGetter } from '$lib/vendor/types.js';
...
function isBox(value: unknown): value is ReadableBox<unknown> {
	return value !== null && typeof value === 'object' && BoxSymbol in value;
}
/**
...
function boxFrom<T>(value: MaybeBoxOrGetter<T>) {
	...
	if (typeof value === 'function') return boxWith(value as Getter<T>);
