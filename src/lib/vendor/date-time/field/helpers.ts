...
import { ALL_SEGMENT_PARTS, DATE_SEGMENT_PARTS, EDITABLE_SEGMENT_PARTS, EDITABLE_TIME_SEGMENT_PARTS } from './parts';
import { getSegments } from './segments';
import { useId } from '$lib/vendor/use-id';
import { kbd } from '$lib/vendor/kbd';
import type { Granularity, HourCycle } from '$lib/shared/date/types';
...
function getUsedSegments(fieldNode: HTMLElement | null) {
	if (!(typeof document !== 'undefined') || !fieldNode) return [];
...
export function isAcceptableSegmentKey(key: string) {
	const acceptableSegmentKeys = [kbd.ENTER, kbd.ARROW_UP, kbd.ARROW_DOWN, kbd.ARROW_LEFT, kbd.ARROW_RIGHT, kbd.BACKSPACE, kbd.SPACE];
	if (acceptableSegmentKeys.includes(key)) return true;
	if (!Number.isNaN(Number(key)) && !Number.isNaN(Number.parseFloat(key))) return true;
	return false;
}
