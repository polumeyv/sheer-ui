<script lang="ts" generics="TData, TValue">
	...
	import { Skeleton } from '@polumeyv/ui/skeleton';
	...
	type DataTableProps<TData, TValue> = {
		...
		disabled?: boolean;
		toolbar?: Snippet<[{ disabled: boolean }]>;
		actions?: Snippet<[{ selectedRows: TData[]; disabled: boolean }]>;
		empty?: Snippet;
	...
	<div class="flex items-center gap-2 pb-4">
		<InputGroup.Root class="w-64">
			...
			<InputGroup.Input
				...
				oninput={(e: Event) => table.getColumn(searchColumn)?.setFilterValue((e.currentTarget as HTMLInputElement).value)} />
		</InputGroup.Root>
		{#if toolbar}
			{@render toolbar({ disabled: isDisabled })}
		{/if}
		<DropdownMenu.Root>
			<DropdownMenu.Trigger disabled={isDisabled}>
				{#snippet child({ props })}
					<Button variant="outline" disabled={isDisabled} {...props}>Columns</Button>
				{/snippet}
			</DropdownMenu.Trigger>
			<DropdownMenu.Content align="end" class="min-w-48! w-48">
				{#each table.getAllColumns().filter((col) => col.getCanHide()) as column (column.id)}
					<DropdownMenu.CheckboxItem class="capitalize" checked={column.getIsVisible()} onCheckedChange={(v: boolean | 'indeterminate') => column.toggleVisibility(!!v)}>
						{column.id}
					...
					{#each table.getRowModel().rows as row (row.id)}
						<Table.Row
							...
							onclick={(e: MouseEvent) => {
								...
								if (target.closest('[role="checkbox"]') || target.closest('button') || target.closest('a')) return;
	...
	{#if showPagination}
		<div class="flex items-center justify-between gap-2 pt-4">
			<div class="flex items-center gap-2">
				...
				<Select.Root type="single" value={String(pagination.pageSize)} onValueChange={(v: string) => table.setPageSize(Number(v))}>
