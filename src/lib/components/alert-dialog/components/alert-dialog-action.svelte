<script lang="ts">
	...
	let { children, child, id = createId(uid), ref = $bindable(null), ...restProps }: AlertDialogActionProps = $props();
	...
	const mergedProps = $derived(mergeProps({ 'data-slot': 'alert-dialog-action', class: buttonVariants() }, restProps, actionState.props));
</script>
