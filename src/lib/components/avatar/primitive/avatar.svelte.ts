...
import type { AvatarImageLoadingStatus } from "$lib/components/avatar/primitive/types.js";
...
export class AvatarRootState {
	static create(opts: AvatarRootStateOpts) {
		return setAvatarRootContext(new AvatarRootState(opts));
	}
...
export class AvatarImageState {
	static create(opts: AvatarImageStateOpts) {
		return new AvatarImageState(opts, getAvatarRootContext());
	}
