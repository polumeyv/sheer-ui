...
export const NodeHandler = (root?: HTMLElement | null): NodeHandlerType => {
	...
	const getNodes = (options: OptionsType): NodesType => {
		...
		const containerNode = typeof userContainer === 'string' ? root.querySelector(userContainer) : userContainer;
		const container = <HTMLElement>(containerNode || root.children[0]);

		const slideNodes = typeof userSlides === 'string' ? container.querySelectorAll(userSlides) : userSlides;
