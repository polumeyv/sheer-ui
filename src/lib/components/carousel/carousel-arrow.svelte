<script lang="ts">
	...
	const horizontal = $derived(
		isNext ? '-inset-e-12 top-1/2 -translate-y-1/2' : '-inset-s-12 top-1/2 -translate-y-1/2',
	);
...
<Button
	...
	class={cn('absolute size-8! rounded-full!', emblaCtx.orientation === 'horizontal' ? horizontal : vertical, className)}
