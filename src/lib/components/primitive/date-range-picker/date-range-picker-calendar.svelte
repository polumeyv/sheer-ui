<script lang="ts">
	...
	import type { DateRangePickerCalendarProps } from '$lib/components/primitive/date-range-picker/index';
	import { getDateRangePickerRootContext } from './date-range-picker.svelte.js';
	import { createId } from '$lib/vendor/create-id';
	import { RangeCalendarRootState } from '$lib/components/primitive/range-calendar/range-calendar.svelte';

	const uid = $props.id();

	let { children, child, id = createId(uid), ref = $bindable(null), ...restProps }: DateRangePickerCalendarProps = $props();
