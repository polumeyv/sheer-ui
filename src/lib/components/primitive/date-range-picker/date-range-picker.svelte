<script lang="ts">
	...
	import type { DateRangePickerRootProps } from '$lib/components/primitive/date-range-picker/index';
	import { PopoverRootState } from '$lib/components/primitive/popover/popover.svelte';
	import { DateRangeFieldRootState } from '$lib/components/primitive/date-range-field/date-range-field.svelte.js';
	...
	const pickerRootState = DateRangePickerRootState.create({
		...
		placeholder: {
			get current() {
				return placeholder as DateValue;
			},
			set current(v) {
				placeholder = v;
