...
import { attachObservedResize, type ResizeAttachment } from '$lib/vendor/svelte-resize-observer.svelte';

const navigationMenuAttrs = createBitsAttrs({
	component: 'navigation-menu',
	parts: ['root', 'sub', 'item', 'list', 'trigger', 'content', 'link', 'viewport', 'menu', 'indicator'],
});

const [getNavigationMenuProviderContext, setNavigationMenuProviderContext] = createContext<NavigationMenuProviderState>();
export const [getNavigationMenuItemContext, setNavigationMenuItemContext] = createContext<NavigationMenuItemState>();
const [getNavigationMenuListContext, setNavigationMenuListContext] = createContext<NavigationMenuListState>();
const [, setNavigationMenuContentContext] = createContext<NavigationMenuContentState>();
...
class NavigationMenuProviderState {
	...
	constructor(opts: NavigationMenuProviderStateOpts) {
		...
		this.onTriggerLeave = opts.onTriggerLeave ?? (() => {});
		this.onContentEnter = opts.onContentEnter ?? (() => {});
		this.onContentLeave = opts.onContentLeave ?? (() => {});
...
export class NavigationMenuSubState {
	static create(opts: NavigationMenuSubStateOpts) {
		return new NavigationMenuSubState(opts, getNavigationMenuProviderContext());
	}
...
export class NavigationMenuItemState {
	static create(opts: NavigationMenuItemStateOpts) {
		return setNavigationMenuItemContext(new NavigationMenuItemState(opts, getNavigationMenuListContext()));
	}
	...
	readonly props = $derived.by(
		() =>
			({
				...
				'data-disabled': Boolean(this.opts.disabled.current) ? '' : undefined,
...
const BITS_LINK_SELECT = 'bitsLinkSelect';
const BITS_ROOT_CONTENT_DISMISS = 'bitsRootContentDismiss';

type BitsLinkSelectEvent = CustomEvent<void>;
type BitsRootContentDismissEvent = CustomEvent<void>;
type RootContentDismissAttachment = Record<symbol, Attachment<HTMLElement>>;

function dispatchLinkSelect(node: EventTarget, onSelect: (event: BitsLinkSelectEvent) => void) {
	const removeListener = on(node, BITS_LINK_SELECT, (event) => onSelect(event as BitsLinkSelectEvent), { once: true });

	try {
		const event = new CustomEvent<void>(BITS_LINK_SELECT, {
			bubbles: true,
			cancelable: true,
		});
...
function dispatchRootContentDismiss(node: EventTarget) {
	return node.dispatchEvent(
		new CustomEvent<void>(BITS_ROOT_CONTENT_DISMISS, {
			bubbles: true,
			cancelable: true,
		}),
	);
...
function attachRootContentDismiss(handler: (node: HTMLElement, event: BitsRootContentDismissEvent) => void): RootContentDismissAttachment {
	return {
		[createAttachmentKey()]: (node) =>
			on(node, BITS_ROOT_CONTENT_DISMISS, (event) => {
				handler(node, event as BitsRootContentDismissEvent);
			}),
	};
}
...
export class NavigationMenuLinkState {
	...
	constructor(opts: NavigationMenuLinkStateOpts, context: { provider: NavigationMenuProviderState; item: NavigationMenuItemState }) {
		this.opts = opts;
		this.context = context;
		this.attachment = attachRef(this.opts.ref);
	}
	...
	onclick = (e: BitsMouseEvent<HTMLAnchorElement>) => {
		...
		const linkSelectEvent = dispatchLinkSelect(currTarget, (event) => {
			this.opts.onSelect.current(event);
		});
...
export class NavigationMenuIndicatorImplState {
	...
	readonly activeTriggerResizeAttachment: ResizeAttachment;
	readonly indicatorTrackResizeAttachment: ResizeAttachment;
	readonly opts: NavigationMenuIndicatorStateOpts;
	readonly attachment: RefAttachment;
	context: NavigationMenuProviderState;
	listContext: NavigationMenuListState;
	position = $state.raw<{ size: number; offset: number } | null>(null);
	readonly isHorizontal = $derived.by(() => this.context.opts.orientation.current === 'horizontal');
	...
	constructor(
		...
		this.activeTriggerResizeAttachment = attachObservedResize(() => this.activeTrigger, this.handlePositionChange);

		this.indicatorTrackResizeAttachment = attachObservedResize(() => this.context.indicatorTrackRef.current, this.handlePositionChange);
	}
	...
	readonly props = $derived.by(
		() =>
			({
				...
				...this.attachment,
				...this.activeTriggerResizeAttachment,
				...this.indicatorTrackResizeAttachment,
			}) as const,
	);
}
...
export class NavigationMenuContentImplState {
	static create(opts: NavigationMenuContentImplStateOpts, itemState?: NavigationMenuItemState) {
		return new NavigationMenuContentImplState(opts, itemState ?? getNavigationMenuItemContext());
	}
	readonly rootContentDismissAttachment: RootContentDismissAttachment;
	...
	constructor(opts: NavigationMenuContentImplStateOpts, itemContext: NavigationMenuItemState) {
		...
		this.rootContentDismissAttachment = this.context.opts.isRootMenu
			? attachRootContentDismiss((content) => {
	...
	onkeydown = (e: BitsKeyboardEvent) => {
		...
		if (this.itemContext.contentNode) {
			const focusedNode = this.itemContext.contentNode.querySelector<HTMLElement>('[data-focused]');
			if (focusedNode) {
	...
	readonly props = $derived.by(
		() =>
			({
				...
				'data-orientation': this.context.opts.orientation.current,
				'data-state': this.context.opts.value.current === this.itemContext.opts.value.current ? 'open' : 'closed',
				onkeydown: this.onkeydown,
				[navigationMenuAttrs.content]: '',
				...this.attachment,
				...this.rootContentDismissAttachment,
			}) as const,
	);
}
...
export class NavigationMenuViewportState {
	static create(opts: NavigationMenuViewportStateOpts) {
		return new NavigationMenuViewportState(opts, getNavigationMenuProviderContext());
	}
	readonly contentResizeAttachment: ResizeAttachment;
	...
	constructor(opts: NavigationMenuViewportStateOpts, context: NavigationMenuProviderState) {
		 ...
		 */
		this.contentResizeAttachment = attachObservedResize(
			() => this.contentNode,
			(contentNode) => {
				this.size = {
...
function whenMouse<T extends HTMLElement = HTMLElement>(handler: PointerEventHandler<T>): PointerEventHandler<T> {
 ...
 */
function handleProxyFocus(guard: HTMLElement | null, focusOptions?: Parameters<HTMLElement['focus']>[0]) {
	if (!guard) return;
