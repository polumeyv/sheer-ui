...
import { attachRef, getWindow, type ReadableProps, type WritableProps } from '$lib/vendor/index';
import type { Page, PageItem } from '$lib/components/primitive/pagination/index';
...
const [getPaginationRootContext, setPaginationRootContext] = createContext<PaginationRootState>();
...
export class PaginationRootState {
	static create(opts: PaginationRootStateOpts) {
		return setPaginationRootContext(new PaginationRootState(opts));
	}
...
export class PaginationPageState {
	static create(opts: PaginationPageStateOpts) {
		return new PaginationPageState(opts, getPaginationRootContext());
	}
...
export class PaginationButtonState {
	static create(opts: PaginationButtonStateOpts) {
		return new PaginationButtonState(opts, getPaginationRootContext());
	}
...
function handleTriggerKeydown(e: KeyboardEvent, node: HTMLElement | null, root: PaginationRootState) {
	...
	const dir = ((el) => getWindow(el).getComputedStyle(el).direction as Direction)(root.opts.ref.current);
