...
import type { ScrollAreaType } from '$lib/components/primitive/scroll-area/index';
...
export const [getScrollAreaRootContext, setScrollAreaRootContext] = createContext<ScrollAreaRootState>();
export const [getScrollAreaScrollbarContext, setScrollAreaScrollbarContext] = createContext<ScrollAreaScrollbarState>();
export const [getScrollAreaScrollbarVisibleContext, setScrollAreaScrollbarVisibleContext] =
	createContext<ScrollAreaScrollbarVisibleState>();
export const [getScrollAreaScrollbarAxisContext, setScrollAreaScrollbarAxisContext] = createContext<ScrollbarAxis>();
export const [getScrollAreaScrollbarSharedContext, setScrollAreaScrollbarSharedContext] = createContext<ScrollAreaScrollbarSharedState>();
...
export class ScrollAreaRootState {
	static create(opts: ScrollAreaRootStateOpts) {
		return setScrollAreaRootContext(new ScrollAreaRootState(opts));
	}
...
export class ScrollAreaViewportState {
	static create(opts: ScrollAreaViewportStateOpts) {
		return new ScrollAreaViewportState(opts, getScrollAreaRootContext());
	}
...
export class ScrollAreaScrollbarState {
	static create(opts: ScrollAreaScrollbarStateOpts) {
		return setScrollAreaScrollbarContext(new ScrollAreaScrollbarState(opts, getScrollAreaRootContext()));
	}
...
export class ScrollAreaScrollbarAutoState {
	...
	readonly scrollbar: ScrollAreaScrollbarState;
	readonly root: ScrollAreaRootState;
	readonly viewportResizeAttachment: ResizeAttachment;
	...
	constructor(scrollbar: ScrollAreaScrollbarState) {
		...
		this.viewportResizeAttachment = attachObservedResize(() => this.root.viewportNode, handleResize);

		this.contentResizeAttachment = attachObservedResize(() => this.root.contentNode, handleResize);
	}
...
export class ScrollAreaScrollbarXState implements ScrollbarAxisState {
	static create() {
		return setScrollAreaScrollbarAxisContext(new ScrollAreaScrollbarXState(getScrollAreaScrollbarVisibleContext()));
	}
	...
	constructor(scrollbarVis: ScrollAreaScrollbarVisibleState) {
		...
		$effect(() => {
			// the ref is populated by an attachment, so its presence means the
			// element is mounted in the DOM and ready to measure.
...
export class ScrollAreaScrollbarYState implements ScrollbarAxisState {
	static create() {
		return setScrollAreaScrollbarAxisContext(new ScrollAreaScrollbarYState(getScrollAreaScrollbarVisibleContext()));
	}
	readonly scrollbarVis: ScrollAreaScrollbarVisibleState;
	readonly root: ScrollAreaRootState;
	readonly scrollbar: ScrollAreaScrollbarState;
...
export class ScrollAreaScrollbarSharedState {
	...
	readonly scrollbarResizeAttachment: ResizeAttachment;
	readonly contentResizeAttachment: ResizeAttachment;
	...
	readonly maxScrollPos = $derived.by(() => this.scrollbarVis.sizes.content - this.scrollbarVis.sizes.viewport);
	...
	constructor(scrollbarState: ScrollbarAxis) {
		...
		this.scrollbarResizeAttachment = attachObservedResize(() => this.scrollbar.opts.ref.current, this.handleResize);

		this.contentResizeAttachment = attachObservedResize(() => this.root.contentNode, this.handleResize);
	...
	readonly props = $derived.by(() =>
		mergeProps({
			...
			...this.scrollbarResizeAttachment,
			...this.contentResizeAttachment,
			style: {
				position: 'absolute',
...
export class ScrollAreaThumbImplState {
	static create(opts: ScrollAreaThumbImplStateOpts) {
		return new ScrollAreaThumbImplState(opts, getScrollAreaScrollbarSharedContext());
	}
...
export class ScrollAreaCornerImplState {
	static create(opts: ScrollAreaCornerImplStateOpts) {
		return new ScrollAreaCornerImplState(opts, getScrollAreaRootContext());
	}
	readonly scrollbarXResizeAttachment: ResizeAttachment;
	readonly scrollbarYResizeAttachment: ResizeAttachment;
	...
	constructor(opts: ScrollAreaCornerImplStateOpts, root: ScrollAreaRootState) {
		...
		this.scrollbarXResizeAttachment = attachObservedResize(
			() => this.root.scrollbarXNode,
			(scrollbarXNode) => {
				const height = scrollbarXNode.offsetHeight || 0;
				this.root.cornerHeight = height;
				this.#height = height;
			},
		);

		this.scrollbarYResizeAttachment = attachObservedResize(
			() => this.root.scrollbarYNode,
			(scrollbarYNode) => {
				const width = scrollbarYNode.offsetWidth || 0;
	...
	readonly props = $derived.by(() => ({
		...
		...this.attachment,
		...this.scrollbarXResizeAttachment,
		...this.scrollbarYResizeAttachment,
	}));
}
...
function getThumbOffsetFromScroll({ scrollPos, sizes, dir = 'ltr' }: GetThumbOffsetFromScrollProps) {
	const thumbSizePx = getThumbSize(sizes);
	const scrollbarPadding = sizes.scrollbar.paddingStart + sizes.scrollbar.paddingEnd;
	...
	const scrollWithoutMomentum = Math.min(scrollClampRange[1]!, Math.max(scrollClampRange[0]!, scrollPos));
