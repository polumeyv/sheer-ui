...
import type { RangeCalendarRootState } from '$lib/components/primitive/range-calendar/range-calendar.svelte';
...
export const [getCalendarRootContext, setCalendarRootContext] = createContext<CalendarRootState | RangeCalendarRootState>();

export class CalendarRootState {
	static create(opts: CalendarRootStateOpts) {
		return setCalendarRootContext(new CalendarRootState(opts));
	}
...
export class CalendarHeadingState {
	static create(opts: CalendarHeadingStateOpts) {
		return new CalendarHeadingState(opts, getCalendarRootContext());
	}
...
const [getCalendarCellContext, setCalendarCellContext] = createContext<CalendarCellState>();
...
export class CalendarCellState {
	static create(opts: CalendarCellStateOpts) {
		return setCalendarCellContext(new CalendarCellState(opts, getCalendarRootContext() as CalendarRootState));
	}
	...
	readonly sharedDataAttrs = $derived.by(
		() =>
			({
				...
				'data-selected': this.isSelectedDate ? '' : undefined,
				'data-value': this.opts.date.current.toString(),
				'data-type': getDateValueType(this.opts.date.current),
				'data-disabled': this.isDisabled || (this.isOutsideMonth && this.root.opts.disableDaysOutsideMonth.current) ? '' : undefined,
			}) as const,
	);
	...
	readonly props = $derived.by(
		() =>
			({
				id: this.opts.id.current,
				role: 'gridcell',
				'aria-selected': this.isSelectedDate ? 'true' : 'false',
				'aria-disabled': this.ariaDisabled ? 'true' : 'false',
...
export class CalendarDayState {
	static create(opts: CalendarDayStateOpts) {
		return new CalendarDayState(opts, getCalendarCellContext());
	}
	...
	readonly props = $derived.by(
		() =>
			({
				...
				'aria-disabled': this.cell.ariaDisabled ? 'true' : 'false',
...
export class CalendarNextButtonState {
	static create(opts: CalendarNextButtonStateOpts) {
		return new CalendarNextButtonState(opts, getCalendarRootContext());
	}
...
export class CalendarPrevButtonState {
	static create(opts: CalendarPrevButtonStateOpts) {
		return new CalendarPrevButtonState(opts, getCalendarRootContext());
	}
...
export class CalendarGridState {
	static create(opts: CalendarGridStateOpts) {
		return new CalendarGridState(opts, getCalendarRootContext());
	}
	...
	readonly props = $derived.by(
		() =>
			({
				...
				tabindex: -1,
				role: 'grid',
				'aria-readonly': this.root.opts.readonly.current ? 'true' : 'false',
				'aria-disabled': this.root.opts.disabled.current ? 'true' : 'false',
				'data-readonly': this.root.opts.readonly.current ? '' : undefined,
				'data-disabled': this.root.opts.disabled.current ? '' : undefined,
...
export class CalendarGridBodyState {
	static create(opts: CalendarGridBodyStateOpts) {
		return new CalendarGridBodyState(opts, getCalendarRootContext());
	}
...
export class CalendarGridHeadState {
	static create(opts: CalendarGridHeadStateOpts) {
		return new CalendarGridHeadState(opts, getCalendarRootContext());
	}
...
export class CalendarGridRowState {
	static create(opts: CalendarGridRowStateOpts) {
		return new CalendarGridRowState(opts, getCalendarRootContext());
	}
...
export class CalendarHeadCellState {
	static create(opts: CalendarHeadCellStateOpts) {
		return new CalendarHeadCellState(opts, getCalendarRootContext());
	}
...
export class CalendarHeaderState {
	static create(opts: CalendarHeaderStateOpts) {
		return new CalendarHeaderState(opts, getCalendarRootContext());
	}
...
export class CalendarMonthSelectState {
	static create(opts: CalendarMonthSelectStateOpts) {
		return new CalendarMonthSelectState(opts, getCalendarRootContext());
	}
...
export class CalendarYearSelectState {
	static create(opts: CalendarYearSelectStateOpts) {
		return new CalendarYearSelectState(opts, getCalendarRootContext());
	}
