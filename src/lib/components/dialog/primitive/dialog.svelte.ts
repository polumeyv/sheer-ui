...
function getDialogRootContext(): DialogRootState {
	if (!hasContext(dialogRootContextKey))
		throw new Error('Context "Dialog.Root | AlertDialog.Root" not found');
	return getContext<DialogRootState>(dialogRootContextKey);
}
function getDialogRootContextOr<T>(fallback: T): DialogRootState | T {
	return hasContext(dialogRootContextKey)
		? getContext<DialogRootState>(dialogRootContextKey)
		: fallback;
}
