...
const [getTooltipProviderContext, setTooltipProviderContext] = createContext<TooltipProviderState>();
const [getTooltipRootContext, setTooltipRootContext] = createContext<TooltipRootState>();
...
export class TooltipProviderState {
	static create(opts: TooltipProviderStateOpts) {
		return setTooltipProviderContext(new TooltipProviderState(opts));
	}
...
export class TooltipRootState {
	static create(opts: TooltipRootStateOpts) {
		return setTooltipRootContext(new TooltipRootState(opts, getTooltipProviderContext()));
	}
...
export class TooltipTriggerState {
	static create(opts: TooltipTriggerStateOpts) {
		...
		return new TooltipTriggerState(opts, getTooltipRootContext(), null);
	}
...
export class TooltipContentState {
	static create(opts: TooltipContentStateOpts) {
		return new TooltipContentState(opts, getTooltipRootContext());
	}
	...
	onInteractOutside = (e: PointerEvent) => {
		if (e.target instanceof Element && this.root.triggerNode?.contains(e.target) && this.root.disableCloseOnTriggerClick) {
			e.preventDefault();
			return;
		}
	...
	readonly props = $derived.by(
		() =>
			({
				...
				'data-disabled': this.root.disabled ? '' : undefined,
				...{
