import { type VariantProps, tv } from '../../utils';
...
export const buttonVariants = tv({
	...
	variants: {
		variant: {
			default: 'focus-visible:border-ring focus-visible:ring-ring/50 border-transparent bg-primary text-primary-foreground hover:bg-primary/80',
		...
		size: {
			...
			'icon-xs': "text-sm size-6 rounded-[min(var(--radius-md),8px)] in-data-[slot=button-group]:rounded-md [&_svg:not([class*='size-'])]:size-3",
			'icon-sm': "text-sm [&_svg:not([class*='size-'])]:size-4 size-8 rounded-[min(var(--radius-md),10px)] in-data-[slot=button-group]:rounded-md",
