 ...
 * shadow roots, and the display check modes wired by src/lib/internal/tabbable.ts.
 */
...
function isInert(node: Node | null | undefined, lookUp = true): boolean {
	...
	return Boolean(
		inert ||
			(lookUp &&
				node &&
				(typeof (node as Element).closest === 'function' ? (node as Element).closest('[inert]') : isInert(node.parentNode))),
	);
}
...
export function isTabbable(node: Element, options: TabbableOptions = {}) {
	if (!node) throw new Error('No node provided');
	if (!matches.call(node, candidateSelector)) return false;
	return isNodeMatchingSelectorTabbable(options, node as TabbableElement);
}

export function isFocusable(node: Element, options: TabbableOptions = {}) {
	if (!node) throw new Error('No node provided');
