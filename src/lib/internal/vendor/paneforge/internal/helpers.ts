...
export function updateResizeHandleAriaValues({ groupId, layout, panesArray, domContext }: UpdateResizeHandleAriaValuesOpts) {
...
export function getResizeHandleElementsForGroup(groupId: string, domContext: DOMContext): HTMLElement[] {
	if (!BROWSER) return [];
	return Array.from(domContext.querySelectorAll(`[data-pane-resizer-id][data-pane-group-id="${groupId}"]`) as NodeListOf<HTMLElement>);
}
...
export function getResizeHandleElementIndex({ groupId, id, domContext }: GetResizeHandleElementIndexOpts): number | null {
