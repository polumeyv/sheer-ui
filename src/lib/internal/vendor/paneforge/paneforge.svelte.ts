...
import { type PaneGroupStorage, initializeStorage, loadPaneGroupState, updateStorageValues } from './internal/utils/storage.js';
...
const [getPaneGroupContext, setPaneGroupContext] = createContext<PaneGroupState>();
...
export class PaneGroupState {
	static create(opts: PaneGroupStateOpts) {
		return setPaneGroupContext(new PaneGroupState(opts));
	}
	...
	#setResizeHandlerEventListeners = () => {
		...
		const unsubHandlers = handles.map((handle) => {
			...
			const unsubListener = on(handle, 'keydown', onKeydown);
...
export class PaneResizerState {
	static create(opts: PaneResizerStateOpts) {
		return new PaneResizerState(opts, getPaneGroupContext());
	}
	...
	readonly props = $derived.by(
		() =>
			({
				...
				'data-active': this.#isDragging ? 'pointer' : this.#isFocused ? 'keyboard' : undefined,
...
export class PaneState {
	static create(opts: PaneStateOpts) {
		return new PaneState(opts, getPaneGroupContext());
	}
	...
	readonly #paneState = $derived.by(() => (this.#paneTransitionState !== '' ? this.#paneTransitionState : this.#isCollapsed ? 'collapsed' : 'expanded'));
